...
3. **Implement Phase Tasks Sequentially**: For each unchecked task in the identified phase:
   ...
   a. **Launch Task Implementation Agent**: Use the Task tool with subagent_type="task-implementer" for the current task:
      - Provide the specific task to implement
      - Include phase context (phase goal, acceptance criteria)
      - Include full research context if available (check for research.md in same directory)
